"""
DOVE — Three-stream pipeline: backbone + invariant (1440-d) + motion (139-d).

  frames → Backbone          → z_bb  ∈ R^d_bb
  frames → InvariantPipeline → z_inv ∈ R^1440
  frames → MotionPipeline    → z_mot ∈ R^139  (zero for iNaturalist images)

  z_bb, z_inv, z_mot → TripleFusion → z ∈ R^512 → Head → ŷ ∈ R^20

Grid: 4 backbones × 2 fusions × 2 heads = 16 configs.
Results → results_triple/
"""
...
logger = logging.getLogger("run_experiments_triple")

REPO_ROOT      = Path(__file__).parent.parent
RESULTS_DIR    = REPO_ROOT / "results_triple" / "tables"
FIGS_DIR       = REPO_ROOT / "results_triple" / "figures"
CKPT_DIR       = REPO_ROOT / "results_triple" / "checkpoints"
...
# ── Motion lookup (shared with motion-only script) ────────────────────────────

def _build_stem_to_motion() -> dict[str, np.ndarray]:
    if not MOTION_PARQUET.exists():
        logger.warning("Motion parquet not found — motion feats will be zero")
        return {}
    df = pd.read_parquet(MOTION_PARQUET)
    feat_cols = [f"feat_{i}" for i in range(139)]
    out = {}
    for _, row in df.iterrows():
        stem = Path(row["video_src"]).stem
        out[stem] = row[feat_cols].values.astype(np.float32)
    logger.info("Motion lookup: %d stems", len(out))
    return out
...
class TripleConcatFusion(nn.Module):
    """Project each of three streams to 512, cat → Linear(1536,512)+LN."""
    def __init__(self, backbone_dim: int, inv_dim: int = 1440,
                 mot_dim: int = 139, d: int = 512):
        super().__init__()
        self.bb_proj  = nn.Sequential(nn.Linear(backbone_dim, d), nn.GELU())
        self.inv_proj = nn.Sequential(nn.Linear(inv_dim, d), nn.GELU())
        self.mot_proj = nn.Sequential(nn.Linear(mot_dim, d), nn.GELU())
        self.out      = nn.Sequential(nn.Linear(d * 3, d), nn.LayerNorm(d))

    def forward(self, bb, inv, mot):
        return self.out(torch.cat([self.bb_proj(bb),
                                   self.inv_proj(inv),
                                   self.mot_proj(mot)], dim=-1))


class TripleCrossAttentionFusion(nn.Module):
    """
    invariant (Q) attends to backbone (K/V), then motion is concatenated
    before final projection.
    """
    def __init__(self, backbone_dim: int, inv_dim: int = 1440,
                 mot_dim: int = 139, d: int = 512, nhead: int = 8):
        super().__init__()
        self.bb_proj  = nn.Linear(backbone_dim, d)
        self.inv_proj = nn.Linear(inv_dim, d)
        self.mot_proj = nn.Sequential(nn.Linear(mot_dim, d), nn.GELU())
        self.attn     = nn.MultiheadAttention(d, nhead, dropout=0.1, batch_first=True)
        self.norm     = nn.LayerNorm(d)
        self.out      = nn.Sequential(nn.Linear(d * 2, d), nn.LayerNorm(d))

    def forward(self, bb, inv, mot):
        q        = self.inv_proj(inv).unsqueeze(1)
        kv       = self.bb_proj(bb).unsqueeze(1)
        attn_out, _ = self.attn(q, kv, kv)
        attended = self.norm(q + attn_out).squeeze(1)
        return self.out(torch.cat([attended, self.mot_proj(mot)], dim=-1))
...
class TripleStreamModel(nn.Module):
    ...
    def forward(self, images: torch.Tensor, motion_feat: torch.Tensor):
        bb_feat = self.backbone(images)
        if bb_feat.dim() > 2:
            bb_feat = bb_feat.mean(dim=[-2, -1])
        inv_feat = self.inv_pipeline.extract(images)
        fused    = self.fusion(bb_feat, inv_feat, motion_feat)
        return self.head(fused)


def build_model(config: dict, device: torch.device) -> TripleStreamModel:
    ...
    if config["fusion"] == "cross_attention":
        ...
        fusion = TripleConcatFusion(backbone_dim=backbone.feature_dim).to(device)
    ...
    return TripleStreamModel(backbone, inv_pipeline, fusion, head).to(device)
...
def make_loaders(stem_to_motion, batch_size=32):
    ...
    kw = dict(stem_to_motion=stem_to_motion)
    train_ds = MotionAwareDOVEDataset(splits / "train.csv",
                                      transform=get_image_transform(True), **kw)
    val_ds   = MotionAwareDOVEDataset(splits / "val.csv",
                                      transform=get_image_transform(False), **kw)
...
def main():
    ...
    stem_to_motion = _build_stem_to_motion()
    ...
    configs = [{"backbone": bb, "fusion": fu, "head": hd,
                "name": f"triple_{bb}_{fu}_{hd}"}
               for bb, fu, hd in itertools.product(BACKBONES, FUSIONS, NEURAL_HEADS)]

    all_results = []
    results_csv = RESULTS_DIR / "experiment_results_triple.csv"
